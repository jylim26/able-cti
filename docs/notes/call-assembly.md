...
상태를 따라가고, 끝나는 시점을 잡고, 연결·종료 사실을
이벤트로 알리는 것까지다. 아웃바운드, 재동기화는 이후 단계에서 다룬다.
...
번역기는 두 시점에 도메인 이벤트를 발행한다
([ADR-0006](../adr/0006-spring-events-between-modules.md)).

| 시점 | 이벤트 | 실리는 것 |
|---|---|---|
| `AgentConnect` 처리 후 | `CallConnectedEvent` | linkedid, 상담원 인터페이스, 방향 |
| 마지막 `Hangup` 처리 후 | `CallEndedEvent` | linkedid, 상담원 인터페이스(미응답이면 null), 응답 여부 |

이벤트는 `call/event/`의 record이고 도메인 값만 담는다.
첫 구독자는 `agent` 모듈이다 ([상담원 노트](agent.md)의 "콜 이벤트 배선").
